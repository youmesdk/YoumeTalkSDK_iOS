...
{
    id<VoiceEngineCallback> delegate = [YMVoiceService getInstance].delegate;
    SEL sel = @selector(onMemberChange:changeList:isUpdate:);
...
- (YouMeErrorCode_t) setWhiteUserList:(NSString*) channelID  whiteUserList:(NSString*) whiteUserList
{
    ...
    return IYouMeVoiceEngine::getInstance ()->setWhiteUserList([channelID UTF8String], [whiteUserList UTF8String]);
}
...
- (YouMeErrorCode_t) setReportBufferTime:(int)nTimeSec
{
    return IYouMeVoiceEngine::getInstance()->setReportBufferTime( nTimeSec );
}

- (YouMeErrorCode_t) reportAudio
{
    return IYouMeVoiceEngine::getInstance()->reportAudio();
}
